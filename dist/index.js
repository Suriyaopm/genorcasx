...
async function registerRoutes(app2) {
  app2.post("/api/tools/tokenize", async (req, res) => {
    try {
      const { text: text2, language = "english", strategy = "word" } = req.body;
      if (!text2) {
        return res.status(400).json({ error: "Text is required" });
      }
      let tokens = [];
      if (strategy === "sentence") {
        tokens = text2.split(/[.!?]+/).map((sentence) => sentence.trim()).filter((sentence) => sentence.length > 0);
      } else if (strategy === "aggressive") {
        tokens = text2.toLowerCase().replace(/[^\w\s]/g, " ").split(/\s+/).filter((token) => token.length > 0);
      } else {
        tokens = text2.split(/\s+/).filter((token) => token.trim().length > 0);
      }
      const result = {
        original_text: text2,
        language,
  ...
  app2.post("/api/tools/chunk", async (req, res) => {
    try {
      ...
      while (start < text2.length) {
        let end = Math.min(start + chunkSizeNum, text2.length);
        if (end < text2.length) {
          const lastSpace = text2.lastIndexOf(" ", end);
          if (lastSpace > start && lastSpace > start + chunkSizeNum * 0.8) {
            end = lastSpace;
          }
        }
        chunks.push(text2.slice(start, end).trim());
        start = Math.max(start + chunkSizeNum - overlapNum, start + 1);
        if (start >= text2.length) break;
      }
      const result = {
        original_text: text2,
  ...
  app2.post("/api/tools/embed", async (req, res) => {
    try {
      const { text: text2, model = "text-embedding-3-small", dimensions } = req.body;
      if (!text2) {
        return res.status(400).json({ error: "Text is required" });
      }
