...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/tools/tokenize", async (req, res) => {
    try {
      const { text, language = "english", strategy = "word" } = req.body;
      ...
      // Implement reliable tokenization using JavaScript string methods
      if (strategy === "sentence") {
      // Sentence tokenization: split by sentence-ending punctuation
      tokens = text
        .split(/[.!?]+/)
        .map((sentence: string) => sentence.trim())
        .filter((sentence: string) => sentence.length > 0);
      } else if (strategy === "aggressive") {
        // Aggressive tokenization: remove punctuation, lowercase, split by whitespace
        tokens = text
          .toLowerCase()
          .replace(/[^\w\s]/g, ' ')
          .split(/\s+/)
        ...
        // Default word tokenization: split by whitespace, preserve punctuation
        tokens = text
          .split(/\s+/)
      ...
      const result = {
        ...
        tokens,
        statistics: {
          avg_token_length: tokens.reduce((sum, token) => sum + token.length, 0) / tokens.length,
          unique_tokens: [...new Set(tokens)].length,
  ...
  app.post("/api/tools/chunk", async (req, res) => {
    try {
      ...
      while (start < text.length) {
        let end = Math.min(start + chunkSizeNum, text.length);
        
        // Try to break at word boundaries
        if (end < text.length) {
          const lastSpace = text.lastIndexOf(' ', end);
          if (lastSpace > start && lastSpace > start + chunkSizeNum * 0.8) {
            end = lastSpace;
          }
        }
      ...
      const result = {
        ...
        chunks: chunks.map((chunk, index) => ({
          ...
          content: chunk,
          length: chunk.length,
          start_position: text.indexOf(chunk),
          word_count: chunk.split(/\s+/).length
        })),
        statistics: {
          total_characters: text.length,
          avg_chunk_length: chunks.reduce((sum, chunk) => sum + chunk.length, 0) / chunks.length,
          coverage_percentage: (chunks.join('').length / text.length) * 100
        }
      };
  ...
  app.post("/api/tools/embed", async (req, res) => {
    try {
      const { text, model = "text-embedding-3-small", dimensions } = req.body;
      
      if (!text) {
        return res.status(400).json({ error: "Text is required" });
      }

      // For demo purposes, create mock embeddings if no OpenAI key provided
      // In production, you'd require an OpenAI API key
      const mockEmbedding = Array.from({ length: dimensions || 1536 }, () => 
